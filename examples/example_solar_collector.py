...
sc_A = SolarCollector(location=loc, surface_tilt_angle=90, surface_azimuth=-100)
sc_B = SolarCollector(location=loc, surface_tilt_angle=90, surface_azimuth=20)

tims = get_daterange("01-01-2023", "02-01-2023")


clear_sky = loc.get_clear_sky_irad(tims)
solar_pos = loc.get_solar_position(tims)
surface_irad = loc
...
axs[2].plot(tims, sc_A.get_irradiance(tims), label="A")
axs[2].plot(tims, sc_B.get_irradiance(tims), label="B")

axs[1].set_ylabel("surface irad [W/m2]")
axs[2].legend()
