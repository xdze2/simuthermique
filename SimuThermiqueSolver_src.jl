...
"""
    Add a planar 1d wall to the model
    the spatial mesh size (dx) is choosen using the given time step dt and material propeties
    use finite volume approach
    Edge element are half volume: the suface temperature is the node temperature

    surfaces are named "ext" and "int"
"""
function add_simple_wall!(model, wall_id; material, thickness, area, offset=0)
    # Mesh
    delta_x = sqrt( model.dt * material.alpha )
    N = 1 + Int(ceil( thickness/delta_x ))   # at minima two elements
    dx = thickness /( N - 1 )
    
    # Register surfaces node index
    i_ext = length(model.M) + 1 + offset
    i_int = i_ext + N
    ...
    dx_grid = [ (is_boundary(i) ? 0.5 : 1.0)*dx for i in i_ext:i_int ]
...
export Model, Material,
       add_directsource!, add_convectivesource!, add_conductance!,
       add_simple_wall!, add_composite_wall!, add_simplethermalmass!
